...
Notebook 13 directly connects to the COEVO project:
- Shows how the Cetron et al. dataset (50 subjects x 300 parcels x 66 condensed distance vectors) was derived from the pipeline above
- Demonstrates how representational dissimilarity matrices encode the geometry of neural representations
- Links to the COEVO Colab notebooks for the next step (ANN alignment via LoRA/ViT/CLIP)
...
## Relationship to the PURSUE / UAP File Release

The Pentagon's PURSUE release (May 8, 2026) is generating massive public interest in rigorous analysis of anomalous phenomena imagery. The COPP (Citizen Orb Photometry Pipeline) project provides exactly this -- a reproducible photometric analysis pipeline for citizen-captured imagery of unidentified luminous phenomena.

The BIDS Academy model could serve as a template for a parallel "Citizen Science Analysis Academy" covering:
- Photometric analysis (what COPP does)
- Image forensics (detecting compression artifacts vs. genuine phenomena)
- Spectral analysis (if spectroscopic data becomes available from PURSUE)
- Statistical rigor in anomaly detection

The timing is extraordinary. The tools exist. The data is being released. The educational infrastructure to help citizens analyze it rigorously is exactly what's needed.
...
1. Build the interactive ecosystem map (React/D3, deployable to Vercel)
